...
/**
 * \file    propagatedocument/admin/setup.php
 * \ingroup propagatedocument
 * \brief   PropagateDocument setup page.
 */
...
require_once '../lib/propagatedocument.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "propagatedocument@propagatedocument"));
...
$arrayofparameters = array(
	...
	//'PROPAGATEDOCUMENT_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'PROPAGATEDOCUMENT_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'PROPAGATEDOCUMENT_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/propagatedocument/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'PROPAGATEDOCUMENT_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'PROPAGATEDOCUMENT_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$page_name = "PropagateDocumentSetup";
...
$head = propagatedocumentAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "propagatedocument@propagatedocument");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("PropagateDocumentSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'PROPAGATEDOCUMENT_'.strtoupper($myTmpObjectKey).'_ADDON';
